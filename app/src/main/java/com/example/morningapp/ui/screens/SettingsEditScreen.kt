...
import com.example.morningapp.ui.viewmodels.SettingsEditViewModel
import com.example.morningapp.utils.booleanToInt
import com.example.morningapp.utils.intToBoolean

@Composable
fun SettingsEditScreen(navController: NavHostController) {
    val viewModel: SettingsEditViewModel = hiltViewModel()
    val settings by viewModel.settings.collectAsStateWithLifecycle()
    val context = LocalContext.current

    if (settings.name != "") {
        val name = remember { mutableStateOf(settings.name) }
        val location = remember { mutableStateOf(settings.location) }
        val country = remember { mutableStateOf(settings.country) }
        val tTSIsNeeded = remember { mutableStateOf(intToBoolean(settings.tTSIsNeeded)) }
